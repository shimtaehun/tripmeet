...
<table>
  <tr>
    <td width="50%"><img src="https://github.com/user-attachments/assets/8916c4ff-9644-47d5-80df-9749fe973640" width="100%" /></td>
    <td width="50%"><img src="https://github.com/user-attachments/assets/fac85755-20b6-4a78-83f9-63ee5f1c4c3e" width="100%" /></td>
  </tr>
  <tr>
    <td width="50%"><img src="https://github.com/user-attachments/assets/402c1647-6b78-4cd3-bd33-e95b29eef0d5" width="100%" /></td>
    <td width="50%"><img src="https://github.com/user-attachments/assets/92e67910-a8d1-43c3-8d2d-5866ec8e467a" width="100%" /></td>
  </tr>
  <tr>
    <td width="50%"><img src="https://github.com/user-attachments/assets/133e869e-2001-4c24-8ceb-e69514d1d068" width="100%" /></td>
    <td width="50%"><img src="https://github.com/user-attachments/assets/2a8ef61f-e993-478b-9ee7-6b9145391c9e" width="100%" /></td>
  </tr>
</table>
...
처음엔 캐시 키에 예산을 원 단위 그대로 넣었는데, 25만원과 28만원 요청이 매번 캐시 미스가 나면서 Gemini를 새로 호출했습니다. 사실상 같은 여행인데 키만 다른 셈이었죠. 그래서 예산을 `10만원대 / 20만원대 / 30만원대`처럼 구간으로 정규화한 뒤 키를 만들었습니다.
...
이 한 단계로 비슷한 조건의 요청이 캐시를 공유하게 돼서, 일정 생성 API 호출이 눈에 띄게 줄었습니다. 키 끝에 붙은 `:v2`는 식사 슬롯 + RAG 3종을 반영하면서 프롬프트가 바뀐 버전 표시로, 예전 캐시와 섞이지 않도록 의도적으로 끊은 흔적입니다.

### RAG 소스는 실패해도 일정 생성을 막지 않게 했다

AI 일정은 앱에 쌓인 맛집·후기·동행 데이터 5종을 끌어다 프롬프트에 넣습니다. 그런데 이 조회 중 하나라도 에러가 나면 일정 자체가 안 만들어지는 건 과한 결합이라고 봤습니다. RAG는 어디까지나 "품질을 높이는 부가 정보"지 필수 입력이 아니니까요.
...
각 소스 조회를 독립적으로 감싸서, 일부가 비어도 나머지 컨텍스트로 일정은 정상 생성되도록 했습니다. 데이터가 쌓일수록 추천이 좋아지되, 데이터가 없거나 조회가 실패해도 서비스는 멈추지 않는 구조입니다.

### 이미지는 무조건 클라이언트에서 먼저 압축한다

맛집 사진을 원본 그대로 올리면 업로드도 느리고 R2 저장 비용도 커집니다. 그래서 업로드 진입점을 한 군데(`compressImage`)로 모으고, 모든 이미지가 반드시 이 함수를 거치도록 했습니다. 최대 1280px로 리사이즈한 뒤 500KB를 넘으면 품질을 0.2씩 낮춰 최대 3회 재시도합니다.

서버가 아니라 클라이언트에서 줄인 이유는, 애초에 큰 파일이 네트워크를 타지 않게 하는 게 사용자 체감 속도에도 비용에도 유리했기 때문입니다.
