...
def _call_gemini(
    ...
    """RAG 컨텍스트 4종을 주입해 Gemini로 여행 일정을 생성한다."""
...
def get_or_generate_itinerary(
    ...
    """
    RAG 4종(맛집·커뮤니티·시즌·사용자 선호도)을 조합해 일정 생성.
    ...
    logger.info(
        "RAG 조회 완료 | 맛집: %d건 | 커뮤니티팁: %d건 | 시즌: %s | 선호도: %s",
        len(restaurants), len(community_tips),
        bool(season_info), bool(user_preference),
    )

    content = _call_gemini(
        destination, duration_days, travelers_count, budget_range,
        restaurants, community_tips, season_info, user_preference,
    )
